...
public class BarChartRenderer extends DataRenderer {
    ...
    /**
     *
     * @param yPrev y of the previous bar's top
     * @param yFinal y of the final bar's top
     * @param valuePositive true if value is positive
     * @return
     */
    boolean isFinalBarShorterThanPreviousOrEqual(float yPrev, float yFinal, boolean valuePositive) {
        return (valuePositive  == true) ? (yPrev <= yFinal) : (yPrev >= yFinal);
    }

    /**
     * When we have two bars' tops connected by smooth curve, we have to create two additional
     * points to keep the curve smooth also in the junction point of two bars.
     * @param inCoordinates array of coordinates of three points: left bar's top right corner,
     * right bar's top left corner, right bar's top right corner.
     * @return the coordinates of two additional points (left one and right one respectively)
     */
    static float[] CalculateAdditionalPointsBetweenBars(float[] inCoordinates) {
        final float p1x = inCoordinates[0];
        final float p1y = inCoordinates[1];
        final float p2x = inCoordinates[2];
        final float p2y = inCoordinates[3];
        final float p3x = inCoordinates[4];
        final float p3y = inCoordinates[5];
        float[] outCoords = new float[4];
        outCoords[0] = (9 * p2x + p1x)/10;
        outCoords[1] = (9 * p2y + p1y)/10;
        outCoords[2] = (9 * p2x + p3x)/10;
        outCoords[3] = (9 * p2y + p3y)/10;
        return outCoords;
    }
